...
def get_args():
    ...
    if (parsed_args["date"] is None) and (parsed_args["inputfile"] is None):
        parser.error("either --date or --inputfile needs to be set. (--date not yet implemented)")
...
try:
    git_module_version = sp.check_output(["git", "describe", "--always"]).strip()
except:
    git_module_version = "--"
...
for ifile in range(0, len(filelist)):
    ...
    with open(filelist[ifile], 'rb') as fa:
        for line in fa:  # loop over all lines within ASCII file
            ...
            if linecount == 2:
                ...
                timest = string[-1][11:13] + string[-1][14:16] + string[-1][17:19]
            if linecount == 3:
                serial = string[-1]
            if linecount == 4:
                sondetype = string[-1]
    ...
    data = np.genfromtxt(filelist[ifile], skip_header=9, invalid_raise=False, encoding="ISO-8859-1")
    ...
    YYYYMM = num2date(np.int(utctime), 'seconds since 1970-01-01 00:00:00 UTC').strftime('%Y%m')
    YYYYMMDDHHMM = num2date(np.int(utctime), 'seconds since 1970-01-01 00:00:00 UTC').strftime('%Y%m%d%H%M')
    outpath = args['outputfolder'] + YYYYMM + '/'
    if not os.path.exists(outpath):
        success = sp.call(["mkdir", "-p", outpath])
    outfile = outpath + "rs__Vaisala__{}check6.nc".format(time_resolution, YYYYMMDDHHMM)
    ...
    fo.history = 'File created ' + time.ctime(time.time()) + ' with ' + os.path.basename(__file__), \
                 ', last program modification on ' + time.ctime(os.path.getmtime(os.path.realpath(__file__)))
    ...
    nc_tindex.units = 's'
    nc_vvert = fo.createVariable('ascentRate', 'f4', ('levels'), fill_value=fillval)
    nc_vvert.long_name = 'Ascent/descent rate (vertical velocity)'
    nc_vvert.units = 'm/s'
    nc_alti = fo.createVariable('altitude', 'f4', ('levels'), fill_value=fillval)
    nc_alti.long_name = 'altitude'
    nc_alti.units = 'm'
    nc_pres = fo.createVariable('pressure', 'f4', ('levels'), fill_value=fillval)
    nc_pres.long_name = 'pressure'
    nc_pres.units = 'hPa'
    nc_temp = fo.createVariable('temperature', 'f4', ('levels'), fill_value=fillval)
    ...
    nc_rh.units = '%'
    nc_dewp = fo.createVariable('dewPoint', 'f4', ('levels'), fill_value=fillval)
    nc_dewp.long_name = 'dew point'
    nc_dewp.units = 'degrees Celsius'
    nc_mix = fo.createVariable('mixingRatio', 'f4', ('levels'), fill_value=fillval)
    nc_mix.long_name = 'Water vapor mixing ratio'
    nc_mix.units = 'g/kg'
    nc_vhori = fo.createVariable('windSpeed', 'f4', ('levels'), fill_value=fillval)
    nc_vhori.long_name = 'wind speed)'
    nc_vhori.units = 'm/s'
    nc_vdir = fo.createVariable('windDirection', 'f4', ('levels'), fill_value=fillval)
    nc_vdir.long_name = 'wind direction'
    nc_vdir.units = 'degrees'
    nc_lat = fo.createVariable('latitude', 'f4', ('levels'), fill_value=fillval)
    nc_lat.long_name = 'latitude'
    nc_lat.units = 'degrees-north'
    nc_long = fo.createVariable('longitude', 'f4', ('levels'), fill_value=fillval)
